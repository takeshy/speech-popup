...
5. 最後に合図の言葉（初期値は聞き取り言語に連動。日本語は `over` / `オーバー`）を話すと、その語を除いて 3〜4 を自動で行います
...
**コピーして閉じる合図**の初期値は聞き取り言語に連動します。たとえば英語は `over`、フランス語は `terminé`、ドイツ語は `fertig`、日本語は `over, オーバー` です。自由に編集でき、空欄で無効化、**この言語の初期値に戻す**でリセットできます。編集値は言語ごとに記憶し、**保存**すると再起動後も復元します。同じ言語ならサービスや地域をまたいで共通です（繁体字中国語は別）。既存のカスタム合図は現在の聞き取り言語の設定として保持します。聞き取り言語が自動検出の場合は WebView の優先言語を合図の初期値に使い、認識結果ごとには切り替えません。
...
認識サービスが返した `。`・`、` などの句読点はそのまま残します。発話末尾の **クエスチョン／クエスチョンマーク／question／question mark** だけを、認識確定時に `?` へ変換します。手動停止や無音設定が0秒の場合も変換します。それ以外の従来の記号・改行コマンドは普通の文章として扱います。改行は Shift+Enter で入力できます。コピーして閉じる合図は引き続き使えます。
