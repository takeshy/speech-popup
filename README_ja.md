...
置換ルールは、話したフレーズを音声では直接入力しにくい `/daily` などの文字列へ変換します。設定画面で発話と置換後の文字列を1行ずつ登録できます。詳しい一致条件は[挿入位置と句読点](#挿入位置と句読点)を参照してください。

![設定画面の置換ルール](docs/replacements_ja.png)
...
3. 認識結果がテキストエリアに入る。そのまま手で直せます。設定した**置換ルール**も確定時に適用されます（例: `日記書いて` → `/daily`）
4. `Enter` (または Copy) → クリップボードにコピーして窓が閉じ、直前のウィンドウへ貼り付けられます
5. 最後に合図の言葉（初期値は聞き取り言語に連動。日本語は `これで終わります`）を話すと、その語を除いて 3〜4 を自動で行います
...
認識に失敗しても録音は保持されるので、`Ctrl+R` で同じ音声を新しいリクエストとして再送できます。失敗後に新しく録音を開始した場合は、失敗した保持音声を破棄し、新しい音声だけを送ります。認識が成功すると保持分の録音は消費されます。コピーに成功した時点で入力内容と残りの録音を破棄します。
...
`speech-popup show --append "送信して"` のように `--append` を付けると、そのポップアップがコピーするときに末尾へその文言を足します。何も話さずに Enter を押した場合も文言だけをコピーするので、呼び出し側は「自分が出したポップアップからの貼り付けか」「ユーザーが何も言わずに閉じたか」を判別できます。文言はそのポップアップ限りで、閉じるか、オプション無しで開き直すと消えます。Obsidian の LLM Hub チャットは専用の印（`⟦voice-chat⟧`）を渡し、受け取った貼り付けが自分の開いたポップアップからのものだと判別します。印の前に文章があれば回答として送信、何も無ければ会話モードを終了、会話が終わったあとに届いた場合は印だけを外して入力欄に残します。
...
# 認識結果を書き換える。1行1件で "発話 => 置換後" (音声では言えない /daily などのため)
...
**音声コマンド**は **?・改行・!・コピーして閉じる** の4種類です。フレーズは聞き取り言語ごとに記憶し、言語やサービスを切り替えると復元します。初期値を用意するのは日本語（クエスチョン／クエスチョンマーク、エンター、びっくり、これで終わります）と英語（question／question mark、enter、exclamation、I'm done speaking）だけです。ほかの言語は空欄から設定でき、既存の保存済みフレーズは保持します。
...
録音開始から最初の3秒間は無音で自動区切りせず、会話らしい音を検出していない場合も録音を続けます。
...
**置換ルール**は、認識が確定したテキストを書き換えます。設定画面では1ルールが「発話」「置換後」の2欄1行で、**＋ ルールを追加**と各行の**×**で編集します。音声では出せないテキストのための機能です。日本語で `/daily` と言うのはほぼ不可能なので、発話に `日記書いて`・置換後に `/daily` を入れておけば、文章として発話してコマンドとして貼り付けられます。置換後には改行を含む文章も登録できます。長い語から先に一致し、出現箇所はすべて置換し、残りの発話はそのまま後ろに続きます（「日記書いて 今日は雨」→「/daily 今日は雨」）。認識が付けた句読点はフレーズの直後の分だけ一緒に取り除くので、文として発話しても `/infographic。` ではなく `/infographic` になり、後ろに発話が続く場合は空白1つで区切ります。ラテン文字のフレーズは単語単位・大文字小文字を無視して一致し、置換後を空にするとその語を削除します。ルールは聞き取り言語で共通で、`config.toml` には1行1ルールで保存され（置換後の改行はエスケープされます）、全体で4000文字までです。
...
録音方式では無音で録音を区切って送信し、前の発話を認識している間も録音を続けます。無音かどうかは固定のしきい値ではなく部屋の音量を基準に判定します。直近5秒でもっとも静かな瞬間を床とし、そこから明確に上がった音を発話、床の近くまで戻ったら無音とみなします。ファンやキーボード、隣の会話がある部屋でも無音に到達し、静かな部屋の小さな声も拾えます。設定できるのは秒数だけです。短い発話が次の文まで溜まらないよう、音量が250ミリ秒途切れず続くことを要求する条件は廃止しました。100ミリ秒以上離れた2回の音量検出が300ミリ秒以内なら発話とみなし、その後に設定した無音秒数が経過すると送信します。待ち時間が0になったわけではありません。単語だけで反応しない場合は、Ctrl+Spaceで手動停止して送信すると、無音判定と認識サービスの影響を切り分けられます。認識精度はサービスにも依存します。
